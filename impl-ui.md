...
6.  **Task F3: `diff` 视图实现**: 开发 `DiffViewer` 组件，集成 `diff2html.js` 库，根据用户选择动态渲染 `diff` 结果。
...
*   **Step 4: 前端 - 核心组件开发 (Task F2, F3, F4)**
    ...
    2.  **`DiffViewer.tsx`**:
        ...
        *   使用 `useEffect` 和 `useRef` 来集成 `diff2html.js`。当 `diffText` 改变时，清空目标 DOM 元素，并调用 `Diff2HtmlUI` 的 `draw()` 方法重新渲染。
    3.  **`TmuxTerminal.tsx`**:
