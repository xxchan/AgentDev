...
- **把系统真实跑起来。** 至少启动一次 UI/后端，务必用后台方式运行（例如在项目根执行 `PORT=3000 cargo run --bin agentdev-ui > /tmp/agentdev-ui.log 2>&1 &` 并记录 PID），随后通过 `tail /tmp/agentdev-ui.log` 等手段观察输出，确认新增日志（如 git 报错）包含完整的命令、退出码和 stderr 摘要。
